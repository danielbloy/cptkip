    ...
    def read(self, size=-1, stop_byte=None):
        while True:
            ...
            if stop_byte is not None:
                try:
                    new_size = self.read_buffer[self.start:].index(
                        stop_byte) + 1  # stop immediately after encountering the stop byte
                    size = min(size, new_size) if size >= 0 else new_size
                except ValueError:
                    pass
            buffer_slice = self.read_buffer[
                self.start:min(self.end, self.start + size) if size >= 0 else self.end]
            if buffer_slice:
                yield buffer_slice
...
class Server:
    def __init__(self, max_request_line_size=4096, max_header_count=50, max_body_bytes=65536,
    ...
    def handle_request(self, target, method, headers, content_length,
                       buffered_client_socket):  # subclass Server and override this method for more customized request handling (e.g., large file uploads)
        ...
        for route_path, route_method, request_handler in self.routes:
            if path == route_path and method == route_method:
                ...
                print("response status", response.status_code, "sending", len(response.body_bytes),
                      "bytes")
    ...
    def process_client_connection(self, buffered_client_socket):
        try:
            ...
            for data in buffered_client_socket.read(size=self.max_request_line_size,
                                                    stop_byte=b'\n'):
                yield
                start_line += data
            ...
            try:
                ...
                return
            for data in self.handle_request(target, method, headers, content_length,
        ...
        while True:
            if len(client_processors) < max_parallel_connections:
                try:
                    ...
                    client_processors.append((time.monotonic(), new_client_socket,
