...
from cptkip.core.log import debug, info, warn, error, stacktrace
from cptkip.task.control import RUNNER_DEFAULT_CALLBACK_FREQUENCY, SCHEDULER_INTERNAL_LOOP_RATIO
from cptkip.task.scheduler import new_scheduled_task, terminate_on_cancel, new_loop_task
...
class Runner:
    ...
    cancelled and the task will terminate. This can be controlled with self.cancel_on_exception.

    The task can be provided a callback which will be called (roughly) the desired number
    of times per second. This callback can perform any task but will typically be used to
    cancel the task. This can be achieved by setting self.cancel = True. The frequency the
    ...
    Where a task errors with an exception, the interactive normally cancels the task. It is
    possible to get the task to restart tasks that error by turning setting
    ...
    def run(self, callback: Callable[[], Awaitable[None]] = None) -> None:
        ...
        The callback can terminate the interactive by setting task.cancel = True.
    ...
    def __new_scheduled_task_handler(self, task: Callable[[], Awaitable[None]]) -> Callable[[], Awaitable[None]]:
        ...
        async def handler() -> None:
            try:
                ...
                error(f'Caught CancelledError exception for scheduled task {task}, cancelling task')
                self.cancel = True

            except Exception as e:
                error(f'Exception: {e} raised by scheduled task {task}, cancelling task')
                stacktrace(e)
                self.cancel = True
    ...
    async def __internal_loop_wait(self) -> None:
        ...
        control the task. It's a fraction of the timed interval.
        """
