...
class TestNode:
    ...
    def test_details_returns_correct_values(self, monkeypatch) -> None:
        ...
        assert set(details.keys()) == {configuration.FIELD_NAME, configuration.FIELD_ROLE,
                                       configuration.FIELD_COORDINATOR}
        assert details[configuration.FIELD_NAME] == "JacKsoN"
        assert details[configuration.FIELD_ROLE] == "RaNdOm"
        assert details[configuration.FIELD_COORDINATOR] == "arbiter"
